...
def calculate_distance(points):
    return [sum(abs(x) for x in point) for point in points]


def main():
    wires = read_wires()
    wires_points = [set(get_wire_points(wire)) for wire in wires]
    common_points = wires_points[0]
    for wire_points in wires_points[1:]:
        common_points = common_points & wire_points
    distances = calculate_distance(common_points)
    print(min(distances))
