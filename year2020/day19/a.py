...
class Rule:
    ...
    def final_match(self, line: str, rule_set) -> bool:
        res = next(self.match_start(line, rule_set), None)
        return res == ""
...
class MultiRule(Rule):
    ...
    def check_subrule(self, line: str, subrule: List[int], rule_set: Dict[int, 'Rule']):
        res = line
        for rule_id in subrule:
            res = next(rule_set[rule_id].match_start(res, rule_set), None)
            if res is None:
                return
        yield res
...
def main():
    ...
    logger.info(f"Matching lines {res}, {sum(res)}")
